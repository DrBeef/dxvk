...
namespace dxvk {
  ...
  D3D9CommonShader::D3D9CommonShader(
    ...
    if (dumpPath.size() != 0) {
      ...
      reader.store(std::ofstream(str::format(dumpPath, "/", name, ".dxso").c_str(),
      ...
      if (SUCCEEDED(hr)) {
        std::ofstream disassembledOut(str::format(dumpPath, "/", name, ".dxso.dis").c_str(), std::ios_base::binary | std::ios_base::trunc);
